...
class E131Packet(object):
    def __init__(self, cid=None, name=None, universe=None, data=[], sequence=0, priority=100):
        self.dmp_packet = DMPLayer(data=data).packet_data()
        self.framing_packet = FramingLayer(name=name, universe=universe,
                dmp_packet=self.dmp_packet,sequence=sequence,priority=priority).packet_data()
